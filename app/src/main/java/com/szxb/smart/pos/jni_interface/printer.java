...
public class printer extends SeriesCom{
	...
	{
		byte[] sendByte = new byte[]{0x1d,0x72,0x01};
		byte[] recvByte = SeriesComTrans(Handle,sendByte,sendByte.length,1000);
		if(null != recvByte && recvByte[0] == 0x0c)
		{
			return 0;
		}
		return 1;
	...
	{
		SeriesComTrans(Handle,arryData,len,0);
		return 0;
	}
